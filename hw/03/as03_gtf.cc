...
vec3 setColor(vec3 pos) {
	...
	for (int i = 0; i<pointLights.size(); i++) {
		...
		// Ambient
//		color += multAcross(circle.ambient, pointLights[i].position);
		...
		// Diffuse
//		color += multAcross(circle1.diffuse, pointLights[i].color)*max(lightVector*normal,0.0f);
		
		// Specular
		color += multAcross(circle1.specular, pointLights[i].color)*pow(max(reflection*viewer,0.0f), circle1.sp);
